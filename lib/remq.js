...
Remq.prototype.publish = function(channel, message, cb) {
  var msg = this.coder.encode(message), seconds = Math.floor(new Date().valueOf() / 1000);
  _evalScript.call(this, publishScript, 0, this.namespace, channel, msg, seconds, cb);
}
...
Remq.prototype.subscribe = function(channel, options) {
  ...
  var timeout = options.interval || 1000
    , cursor = parseFloat(options.cursor, 0) || null
    , self = this;
...
Remq.prototype.consume = function(channel, options, cb) {
  ...
  var cursor = parseFloat(options.cursor, 0) || 0
...
Remq.prototype.purge = function(channel, options, cb) {
  ...
  var before = parseFloat(options.before, 0) || null
