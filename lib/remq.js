...
/**
 * Subscribe to the given `channel`. If no initial `fromId` is provided,
 * Remq subscribes using vanilla Redis pub/sub. Any Redis pub/sub pattern will
 * work. If `fromId` is provided, Remq replays messages after the given id
 * until its caught up and able to switch to pub/sub.
 *
 * Options:
 *
...
Remq.prototype.subscribe = function(channel, options, callback) {
  ...
  if(!cursor) { cursor = this.cursors[channel]; }

  if(cursor || cursor === 0) {
    // if we're resuming, we need to try to catch up from cursor
    this._subscribeFromCursor(channel, cursor, function(err) {
      if(err) { return self._onError(err, callback); }
      if(callback) callback();
    });
  } else {
    // otherwise we can subscribe immediately
    this._subscribeToPubSub(channel, { caughtUp: true }, function(err) {
      if(err) { return self._onError(err, callback); }
      if(callback) callback();
    });
  }
...
Remq.prototype.unsubscribe = function(channel, callback) {
  delete this.subscriptions[channel];
  this._unsubscribeFromPubSub(channel, callback);
};
...
Remq.prototype.consume = function(channel, options, callback) {
  if(!callback) { callback = options, options = {}; }

  var cursor = parseFloat(options.cursor) || this.cursors[channel] || 0
    , limit = parseInt(options.limit, 10) || this.limit
    , self = this;

  _evalScript.call(this, consumeScript, 0, channel, cursor, limit, function(err, messages) {
    if(err) { return self._onError(err, callback); }
    messages = messages.map(self._onMessage.bind(self, self.pattern(channel))).filter(Boolean);
...
Remq.prototype.flush = function(channel, options, callback) {
  if(!callback) { callback = options, options = {}; }
  ...
  _evalScript.call(this, flushScript, 0, channel, before, function (err, removed) {
    if(err) { return self._onError(err, callback); }
...
Remq.prototype._subscribeFromCursor = function(channel, cursor, callback) {
  var self = this;
  (function consumeChannelFrom(cursor) {
    var opts = { cursor: cursor, limit: self.limit };
    self.consume(channel, opts, function(err, results) {
      if(err) { return self._onError(err, callback); }

      if(!self.subscriptions[channel]) { return; } // must have unsubscribed

      var opts = { useBuffer: true, caughtUp: results.length < self.limit };
      self._subscribeToPubSub(channel, opts, function(err, subscribed) {
        if(err) { return self._onError(err, callback); }
        if(subscribed) {
          if(callback) { callback(); }
        } else {
...
/**
 * Subscribe while trying to catch up with pub/sub channel.
 *
 * Options:
 *
...
Remq.prototype._subscribeToPubSub = function(channel, options, callback) {
  ...
  if(this._bufferPubSub && caughtUp) {
    ...
    this._unsubscribeFromPubSub(channel, function(err) {
      self.buffer = [];
    ...
    this.redisPubSub.psubscribe(this.pattern(channel), function(err) {
...
Remq.prototype._unsubscribeFromPubSub = function(channel, callback) {
  var self = this;

  this.redisPubSub.punsubscribe(this.pattern(channel), function(err) {
