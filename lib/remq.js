...
function Remq(options) {
  ...
  this.redis.on('pmessage', function(pattern, channel, message) {
    ...
    self.emit('message', channel.replace(self.channelRegexp, ''), JSON.parse(message));
  });
}
 ...
 * but objects will be automatically serialized and deserialized as JSON. The callback
...
Remq.prototype.publish = function(channel, msg, cb) {
  _evalScript.call(this, publishScript, 0, this.namespace, channel, JSON.stringify(msg), cb);
}
...
Remq.prototype.consume = function(channel, options, cb) {
  ...
  _evalScript.call(this, consumeScript, 0, this.namespace, channel, cursor, limit, function(err, results) {
    ...
    for(var i = 0; i < results.length; i++) {
      if(i % 2 === 0) {
        msgs.push({ id: results[i+1], message: JSON.parse(results[i]) });
      }
    }
    cb(null, msgs);
  });
