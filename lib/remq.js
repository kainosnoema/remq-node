...
var fs = require('fs')
  ...
  , debug = require('debug')('remq:client')
 ...
 * @param {Object} options
 * @return {Remq.ReadableStream} stream
 * @api public
 */

Remq.createReadStream = function(pattern, options) {
  var client = this.createClient(options).subscribe(pattern, options);
  return new Remq.ReadableStream(client, options);
};


/**
 * Create a Writable Stream that publishes data sent to #write() to the given
 * `channel` as a string.
 *
 * Options:
 *
 ...
 * @param {Object} options
 * @return {Remq.WritableStream} stream
 * @api public
 */

Remq.createWriteStream = function(channel, options) {
  return new Remq.WritableStream(channel, this.createClient(options), options);
};
...
Remq.prototype.subscribe = function(pattern, options, callback) {
  if(this.subscriptions[pattern]) return this;
  ...
  if(cursor || cursor === 0) {
    ...
    debug('subscribing to %s from id %s', pattern, cursor);
    ...
    debug('subscribing to %s using pub/sub', pattern);
...
Remq.prototype.consume = function(pattern, options, callback) {
  ...
  _evalScript.call(this, consumeScript, 0, pattern, cursor, limit, function(err, messages) {
    if(err) { return self._onError(err, callback); }
    debug('replaying %d messages on %s from message-id %s', messages.length, pattern, cursor);
...
Remq.prototype.flush = function(pattern, options, callback) {
  ...
  if(before && (before <= 1)) { callback(null, 0); return this; }
...
/**
 * Remove all remq:* keys and reset message id to zero.
 *
 * @param {Function} callback
 * @return {Remq}
 * @api public
...
Remq.prototype.flushAll = function(callback) {
  var self = this;
  this.redis.keys('remq:*', function(err, keys) {
    if(err) { return self._onError(err, callback); }
    if(keys.length) { return self.redis.del(keys, callback); }
...
Remq.prototype.toString = function() {
  return '[Remq host=' + this.redis.host +
...
/**
 * Build a pattern key from the given `channel`.
 *
 * @param {String} channel
...
Remq.prototype._subscribeToPubSub = function(pattern, options, callback) {
  ...
  if(this._bufferPubSub && caughtUp) {
    ...
    this._bufferPubSub = false;
    debug('switching to pub/sub on %s', pattern);
    callback(null, true);
...
Remq.prototype._onMessage = function(pattern, pair) {
  ...
  debug('received message-id %s on %s', message.id, message.channel);
